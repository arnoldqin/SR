...
def define_G(input_nc, output_nc, ngf, which_model_netG, norm='batch', use_dropout=False, init_type='normal', gpu_ids=[], n_downsampling=2, n_upsampling=2, n_resblocks=None, side='A', opt=None):
    netG = None
    ...
    if which_model_netG == 'resnet_9blocks':
        netG = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=n_resblocks, gpu_ids=gpu_ids, n_downsampling=opt.n_downsample, n_upsampling=opt.n_upsample, opt=opt)
    elif which_model_netG == 'resnet_noise':
        netG = ResnetGeneratorNoise(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=n_resblocks, gpu_ids=gpu_ids, n_downsampling=opt.n_downsample, n_upsampling=opt.n_upsample, opt=opt)
    elif which_model_netG == 'resnet_9blocks_old':
        netG = ResnetGeneratorLegacy(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=9, gpu_ids=gpu_ids, opt=opt)
    elif which_model_netG == 'resnet_6blocks':
        netG = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=6, gpu_ids=gpu_ids, opt=opt)
    elif which_model_netG == 'resnet_n_blocks':
        netG = ResnetGenerator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=n_resblocks, gpu_ids=gpu_ids, n_downsampling=n_downsampling, n_upsampling=n_upsampling, opt=opt)
        ...
        netG = Generator(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=3, n_downsampling=n_downsampling, n_upsampling=n_upsampling, gpu_ids=gpu_ids, opt=opt)
        ...
        netG = Generator_core(input_nc, output_nc, mid_nc=opt.pre.ngf, ngf=opt.ngf, n_blocks=n_resblocks, norm_layer=norm_layer, use_dropout=use_dropout, gpu_ids=gpu_ids, n_downsampling=n_downsampling, n_upsampling=n_upsampling, opt=opt)
    elif which_model_netG == 'end2end':
        netG = Generator_end2end(opt=opt)
    elif which_model_netG == 'stack':
        netG = Generator_stack(input_nc, output_nc, ngf, norm_layer=norm_layer, use_dropout=use_dropout, n_blocks=n_resblocks, gpu_ids=gpu_ids, n_downsampling=opt.n_downsample, n_upsampling=opt.n_upsample, side=side, opt=opt)
    else:
        raise NotImplementedError('Generator model name [%s] is not recognized' % which_model_netG)
    if len(gpu_ids) > 0:
        netG.cuda(gpu_ids[0])
    init_weights(netG, init_type=init_type)
    if which_model_netG == 'stack' and opt.load_pre:
        print 'loaded pre-network from', opt.pre_path
        util.load_network_with_path(netG.sub_model, 'G_'+side, opt.pre_path)
    return netG

def define_D(input_nc, ndf, which_model_netD, n_layers_D=3, norm='batch', use_sigmoid=False, init_type='normal', gpu_ids=[], one_out=None, feat_len=None, opt=None):
    netD = None
    use_gpu = len(gpu_ids) > 0
    norm_layer = get_norm_layer(norm_type=norm)
    ...
    if which_model_netD == 'n_layers':
        ...
        netD = MultiscaleDiscriminator(input_nc, ndf, n_layers_D, norm_layer=norm_layer, use_sigmoid=use_sigmoid, num_D=opt.num_D, opt=opt)
...
class ResnetBlock(nn.Module):
    def __init__(self, dim, padding_type, norm_layer, use_dropout, use_bias, shallow=False, use_lrelu=False):
        ...
        self.conv_block = self.build_conv_block(dim, padding_type, norm_layer, use_dropout, use_bias, shallow, use_lrelu)
...
class NNBlock(nn.Module):
    def __init__(self, ch0, ch1, nn_type='conv', kw=3, norm=nn.BatchNorm2d, activation=None, dropout=False, noise=False, pre_activation=False):
...
class ResBlock(nn.Module):
    def __init__(self, dim, norm_layer, activation=nn.LeakyReLU(0.2, True), use_dropout=False, use_bias=True, shallow=False, pre_activation=False):
...
class DownResBlock(nn.Module):
    def __init__(self, ch0, ch1, norm=None, activation=None, dropout=False, noise=False):
        ...
        conv_block = [ResBlock(ch0, norm_layer=norm, activation=activation, use_dropout=dropout, pre_activation=True),
                 ...
                 NNBlock(ch0, ch1, nn_type='down_conv', norm=norm, activation=activation, dropout=dropout, noise=noise, pre_activation=True)]
        self.conv_block = nn.Sequential(*conv_block)
...
class GANLoss(nn.Module):
    def __init__(self, use_lsgan=True, target_real_label=1.0, target_fake_label=0.0,
                 tensor=torch.FloatTensor, target_weight=None):
        ...
        if isinstance(input[0], list):
            ...
            for i, input_i in enumerate(input):
                ...
                if self.target_weight is None:
                    ...
                    loss += self.target_weight[i] * self.loss(pred, target_tensor)
            return loss
        else:            
...
class ColorLoss(nn.Module):
    ...
    def __call__(self, input, target, pass_loss=False):
        ...
        if not self.sig:
            for i in range(3):
                self.loss_color += self.lambda_color_mean * (self.input[:,i,:].mean() - self.target[:,i,:].mean())**2 + \
                               self.lambda_color_var * (self.input[:,i,:].var() - self.target[:,i,:].var())**2
        else:
            self.loss_color +=  self.opt.lambda_color_mean * (self.input.mean() - self.target.mean())**2 + \
                              self.lambda_color_var * (self.input.var() - self.target.var())**2
...
class PerceptualLoss(nn.Module):
    ...
    def trans_norm(self, x):
        rec = x*0.5 + 0.5
        mean = Variable(torch.from_numpy(np.array([0.485, 0.456, 0.406]).reshape((1, 3, 1, 1)).astype('f')).cuda().expand_as(x))
        std = Variable(torch.from_numpy(np.array([0.229, 0.224, 0.225]).reshape((1, 3, 1, 1)).astype('f')).cuda().expand_as(x))
        return (rec-mean)/std

    def __call__(self, input, target, style_layers=4, cont_layer=1, pass_loss=False, lambda_style=None, lambda_content=None):
        if pass_loss:
            return Variable(torch.zeros(1).cuda())
...
class ResnetGenerator(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[], padding_type='reflect', n_downsampling=2, n_upsampling=2, opt=None):
        assert(n_blocks >= 0)
        ...
        for i in range(n_blocks):
            if opt.use_dense:
                ...
                model += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock, use_lrelu = opt.use_lrelu)]
        ...
        if opt.simple_block:
            ...
            model += [nn.Conv2d(last_nc, output_nc, kernel_size=3, padding=0)]
            model += [nn.Tanh()]
        elif opt.not_caffe:
            model += [nn.ReflectionPad2d(3)]
            last_nc = int(ngf * mult)
    ...
    def forward(self, input, y=None, t=None):
        if y is not None:
    ...
    def get_feature(self, input):
        y = input
        for idx in range(len(self.model)-3):
            y = self.model[idx](y)
        self.feat = y
        for idx in range(len(self.model)-3,len(self.model)):
            y = self.model[idx](y)
    ...
    def __init__(self, opt=None):
        ...
        for i in range(n_downsampling):
            ...
            model += [nn.Conv2d(ngf * mult, ngf * mult * fac, kernel_size=3, stride=2, padding=1, bias=use_bias),
                  norm_layer(ngf * mult * fac),
                  activation,
                  ResnetBlock(ngf * mult * fac, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, use_lrelu = opt.use_lrelu)]

        mult = min(2**n_downsampling, opt.max_ngf/ngf)
        for i in range(n_blocks):
            model += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, use_lrelu = opt.use_lrelu)]
        ...
        for i in range(n_upsampling):
            self.up_proc.append(nn.Sequential(*[nn.Conv2d(int(ngf * mult), int(ngf * mult * 2),
                      ...
                      norm_layer(int(ngf * mult / 2)),
                      activation,
                      ResnetBlock(int(ngf * mult / 2), padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, use_lrelu = opt.use_lrelu)]))
            mult /= 2.0
...
class Generator_stack(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[], padding_type='reflect', n_downsampling=2, n_upsampling=2, side='A', opt=None):
        assert(n_blocks >= 0)
        ...
        if not opt.simple_block:
            ...
            for i in range(n_blocks):
                    model += [ResnetBlock(int(ngf // 2), padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock, use_lrelu = opt.use_lrelu)]
            last_nc = int(ngf // 2)
        ...
        self.model = nn.Sequential(*model)
        self.sub_model = define_G(opt.pre.input_nc, opt.pre.output_nc, opt.pre.ngf, opt.pre.which_model_netG, opt.pre.norm, not opt.pre.no_dropout, opt.pre.init_type, self.gpu_ids, n_downsampling=opt.pre.n_downsample, n_resblocks=opt.pre.n_resblocks, side=side, opt=opt.pre)
...
class NLayerDiscriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, use_sigmoid=False, gpu_ids=[], one_out=None, feat_len=None, kw=None, opt=None):
...
class Generator_core(nn.Module):
    def __init__(self, input_nc, output_nc, mid_nc=64, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[], padding_type='reflect', n_downsampling=1, n_upsampling=1, opt=None):
        assert(n_blocks >= 0)
        ...
        for i in range(n_blocks//2):
            pre_model += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock, use_lrelu = opt.use_lrelu)]
        ...
        post_model = [nn.Conv2d(mid_nc, ngf * mult, kernel_size=1, padding=0),
                 ...
                 activation]
        for i in range(n_blocks//2):
            post_model += [ResnetBlock(ngf * mult, padding_type=padding_type, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock, use_lrelu = opt.use_lrelu)]
...
class Generator_stage2only(torch.nn.Module):
    def __init__(self, norm_layer=nn.BatchNorm2d, use_dropout=False, gpu_ids=[], padding_type='reflect', opt=None):
        ...
        output_nc=3
        ngf=64
        self.conv2_1 = NNBlock(input_nc, ngf, nn_type='down_conv7', norm=norm_layer, activation=activation, dropout=False)
        self.conv2_2 = NNBlock(ngf, ngf * 2, nn_type='down_conv', norm=norm_layer, activation=activation, dropout=False)
        self.conv2_3 = NNBlock(ngf * 2, ngf * 4, nn_type='down_conv', norm=norm_layer, activation=activation, dropout=False)
        
        ch = (ngf * 8)
        self.res5 = ResBlock(ch, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock)
        self.res6 = ResBlock(ch, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock)
        self.res7 = ResBlock(ch, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock)
        self.res8 = ResBlock(ch, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock)
        
        self.up2_1 = NNBlock(ch, ch // 2, nn_type='up_'+opt.upsample, norm=norm_layer, activation=activation, dropout=False)
        self.up2_2 = NNBlock(ch // 2, ch // 4, nn_type='up_'+opt.upsample, norm=norm_layer, activation=activation, dropout=False)
...
class Generator(torch.nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[], padding_type='reflect', n_downsampling=2, n_upsampling=2, opt=None):
        ...
        self.conv1_1 = NNBlock(input_nc, ngf, nn_type='down_conv7', norm=norm_layer, activation=activation, dropout=False)
        self.conv1_2 = NNBlock(ngf, ngf * 2, nn_type='down_conv', norm=norm_layer, activation=activation, dropout=False)

        ch = ngf * 2
        self.res1 = ResBlock(ch, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock)
        self.res2 = ResBlock(ch, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock)
        self.res3 = ResBlock(ch, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock)
        self.res4 = ResBlock(ch, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock)
        self.res5 = ResBlock(ch, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock)
        
        self.up1_1 = NNBlock(ch, ch // 2, nn_type='up_'+opt.upsample, norm=norm_layer, activation=activation, dropout=False)
        ...
        self.res7 = ResBlock(ch, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock)
        self.res8 = ResBlock(ch, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock)
        self.res9 = ResBlock(ch, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock)
...
class Generator_tmp(nn.Module):
    def __init__(self, input_nc, output_nc, ngf=64, norm_layer=nn.BatchNorm2d, use_dropout=False, n_blocks=6, gpu_ids=[], padding_type='reflect', n_downsampling=2, n_upsampling=2, opt=None):
        ...
        for i in range(n_downsampling):
            seq += [NNBlock(ch, ch * 2, nn_type='down_conv', norm=norm_layer, activation=activation, dropout=use_dropout)]
            ch *= 2
        for i in range(n_blocks):
            seq += [ResBlock(ch, norm_layer=norm_layer, use_dropout=use_dropout, use_bias=use_bias, shallow = opt.shallow_resblock)]
        for i in range(n_upsampling):
            if opt.upsample == 'conv':
                seq += [NNBlock(ch, ch // 2, nn_type='up_deconv', norm=norm_layer, activation=activation, dropout=use_dropout)]
            if opt.upsample == 'subpix':
                seq += [NNBlock(ch, ch // 2, nn_type='up_subpix', norm=norm_layer, activation=activation, dropout=use_dropout)]
            if opt.upsample == 'scale':
                seq += [NNBlock(ch, ch // 2, nn_type='up_scale', norm=norm_layer, activation=activation, dropout=use_dropout)]
            ch = ch // 2
...
class Discriminator(nn.Module):
    def __init__(self, input_nc, ndf=64, n_layers=3, norm_layer=nn.BatchNorm2d, gpu_ids=[], opt=None):
        ...
        for i in range(1, n_layers):
            ...
            ch*=2
        if opt.one_out:
            seq += [NNBlock(ch * opt.feat_len_D, 1, nn_type='linear', norm=None, activation=nn.Sigmoid() if opt.no_lsgan else None),]
        else:
            seq += [activation,
                 NNBlock(ch, 1, nn_type='conv', norm=None, activation=nn.Sigmoid() if opt.no_lsgan else None, dropout=False, noise=noise),]
