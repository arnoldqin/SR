...
class Sr_base_model(BaseModel):
	...
	self.netE_C = networks.define_G(opt.input_nc, opt.output_nc,
                                        64 ,'ResnetEncoder_my', opt.norm, not opt.no_dropout, opt.init_type, self.gpu_ids, opt=opt, n_downsampling=2)
...
#        networks.print_network(self.netG_B, opt, (opt.input_nc, opt.fineSize, opt.fineSize))
	...
	networks.print_network(self.net_Dc, opt, (opt.ngf, opt.CfineSize/2, opt.CfineSize/2))
	    ...
	    self.fake_C_pool = ImagePool(opt.pool_size)
          ...
          #  self.optimizer_G = torch.optim.Adam(itertools.chain(self.netE_A.parameters(),self.net_D.parameters(),self.netG_A.parameters(),self.net_Dc.parameters(),self.netG_C.parameters()),
	   ...
	   # self.optimizer_AE = torch.optim.Adam(itertools.chain(self.netE_C.parameters(),self.net_D.parameters(),self.net_Dc.parameters(),self.netG_C.parameters()),lr=opt.lr, betas=(opt.beta1, 0.999))
	    self.optimizer_G_A_sr = torch.optim.Adam(itertools.chain(self.netE_A.parameters(),self.net_D.parameters(),self.net_Dc.parameters(),self.netG_C.parameters()),
                                                lr=opt.lr, betas=(opt.beta1, 0.999))
    ...
    def backward_D_A_sr(self):
       ...
       # fake_B = self.fake_B_pool.query(self.fake_B)
        fake_B_sr = self.fake_B_pool.query(self.fake_B_sr)
        if self.opt.lambda_gp > 0:
            self.loss_D_A_real, self.loss_D_A_sr_fake, self.loss_D_A_gp = self.backward_D_basic(self.netD_A, self.real_B, fake_B_sr)
        else:
            self.loss_D_A_real, self.loss_D_A_sr_fake = self.backward_D_basic(self.netD_A, self.real_B_hd, fake_B_sr)
 ...
 #       self.loss_G_A = self.criterionGAN(pred_fake, True) * lambda_adv
	...
	# aux loss
      ...
      #          self.loss_aux_G_A += self.loss_color_A
        ...
        # combined loss
	self.loss_G_l1 = self.criterionCycle(self.fake_B_sr, self.real_B_hd) * self.opt.lambda_AE_A
        self.loss_G = self.loss_G_A_sr + self.loss_G_l1# + self.loss_G_B + self.loss_cycle_A + self.loss_cycle_B + self.loss_idt_A + self.loss_idt_B 
	...
	lambda_AE = self.opt.lambda_AE
	 ...
	 # AE loss
        ...
        self.fake_C = self.net_Dc.forward(self.fake_C)
        self.fake_C = self.netG_C.forward(self.fake_C)
        self.loss_AE = self.criterionCycle(self.fake_C, self.real_C) * lambda_AE
    ...
    def backward_AE_sr(self):
     ...
     #   real_C_tmp = transforms.ToPILImage()(self.real_C).convert('RGB') 
        self.loss_AE_sr = self.criterionCycle(self.fake_C_sr, self.real_C_sr) * lambda_AE
    ...
    def optimize_parameters(self):
        ...
        self.forward()
        # G_A and G_B
        self.optimizer_G_A_sr.zero_grad()
        self.backward_G()
        self.optimizer_G_A_sr.step()
...
#	AE = self.loss_AE.item()
        ret = OrderedDict([('D_A_real', D_A_real),  ('G_A', G_A)])
	    ...
	    g_G_c = util.get_grads(self.netG_C, ret_type='sum').item()
          #  g_G_B = util.get_grads(self.netG_B, ret_type='sum').item()
            ret = OrderedDict(list(ret.items()) + [('D_A_grad', g_D_A), ('E_A_grad', g_E_A),('D_grad', g_D) ('G_A_grad', g_G_A), ('E_C_grad', g_E_C), ('G_C_grad', g_G_C)])
    ...
    def get_current_lr(self):
       ...
       # lr_B = self.optimizer_D_B.param_groups[0]['lr']
        lr_G = self.optimizer_G_A_sr.param_groups[0]['lr']
    ...
    #    running_fake_B = self.netG_A_running.forward(self.real_A)
     #   running_fake_A = self.netG_B_running.forward(self.real_B)
        ...
        fake_B = util.tensor2im(self.fake_B_sr.data,type='big')
    #    running_fake_B_img = util.tensor2im(running_fake_B.data)
    #    rec_A = util.tensor2im(self.rec_A.data)
        real_B = util.tensor2im(self.real_B_hd.data,type='big')
