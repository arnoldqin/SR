...
class CycleMultiDModel(CycleGANModel):
    ...
    def initialize(self, opt):
        ...
        if self.isTrain:
            ...
            self.netD_1A = networks.define_D(opt.output_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids, one_out=True, 
                                            opt=opt)
            self.netD_1B = networks.define_D(opt.input_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids, one_out=True,
                                            opt=opt)
            self.netD_A = networks.define_D(opt.output_nc, opt.ndf,
                                            opt.which_model_netD,
                                            opt.n_layers_D, opt.norm, use_sigmoid, opt.init_type, self.gpu_ids, one_out=False,
                                            opt=opt)
        ...
        if self.isTrain:
            ...
            self.optimizer_D_1A = torch.optim.Adam(self.netD_1A.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizer_D_1B = torch.optim.Adam(self.netD_1B.parameters(), lr=opt.lr, betas=(opt.beta1, 0.999))
            self.optimizers = []
            self.schedulers = []
    ...
    def backward_D_A(self):
        ...
        self.loss_D_1A_real, self.loss_D_1A_fake = self.backward_D_basic(self.netD_1A, self.real_B, fake_B)

    def backward_D_B(self):
        fake_A = self.fake_A_pool.query(self.fake_A)
        self.loss_D_B_real, self.loss_D_B_fake = self.backward_D_basic(self.netD_B, self.real_A, fake_A)
        self.loss_D_1B_real, self.loss_D_1B_fake = self.backward_D_basic(self.netD_1B, self.real_A, fake_A)
    ...
    def backward_G(self):
        ...
        self.loss_G_A = (self.criterionGAN(pred_fake, True) + self.criterionGAN(pred_1fake, True)) * lambda_adv
        # D_B(G_B(B))
        self.fake_A = self.netG_B.forward(self.real_B)
        pred_fake = self.netD_B.forward(self.fake_A)
        pred_1fake = self.netD_1B.forward(self.fake_A)
        self.loss_G_B = (self.criterionGAN(pred_fake, True) + self.criterionGAN(pred_1fake, True)) * lambda_adv
        # Forward cycle loss
        self.rec_A = self.netG_B.forward(self.fake_B)
    ...
    def get_current_errors(self):
        ...
        if self.opt.identity > 0.0:
            ...
            return OrderedDict([('D_A', D_A), ('D_1A', D_1A), ('G_A', G_A), ('Cyc_A', Cyc_A), ('idt_A', idt_A),
                                ('D_B', D_B), ('D_1B', D_1B), ('G_B', G_B), ('Cyc_B', Cyc_B), ('idt_B', idt_B)])
        else:
            return OrderedDict([('D_A', D_A), ('D_1A', D_1A), ('G_A', G_A), ('Cyc_A', Cyc_A),
                                ('D_B', D_B), ('D_1B', D_1B), ('G_B', G_B), ('Cyc_B', Cyc_B)])
