...
class CycleStackv2FinalModel(CycleGANModel):
    def name(self):
        return 'CycleStackv2FinalModel'
    ...
    def initialize(self, opt):
        ...
        if self.isTrain and not opt.test:
            ...
            self.criterionGAN = networks.GANLoss(use_lsgan=not opt.no_lsgan, tensor=self.Tensor, target_weight = self.weight_adv)
            self.criterionCycle = networks.RECLoss(target_weight = self.weight_rec)
    ...
    def optimize_parameters(self, total_iter=None):
        ...
        # self.real_A, self.real_B, self.fake_A, self.fake_B, self.rec_A, self.rec_B
